...
import AdslDetails from '../../../src/app/components/Elife/AdslDetails';
...
describe('AdslDetails Integration Test', () => {
  it('renders DataPanel with correct header when permission is granted', () => {
    ...
    render(
      <Provider store={store}><AdslDetails /></Provider>
    );

    // Check DataPanel header
    expect(screen.getByTestId('ADSL Details')).toBeInTheDocument();
  });
});