...
import PstnDetail from '../../../src/app/components/Elife/PstnDetail';
...
describe('PstnDetail Integration Test', () => {
  it('renders DataPanel with correct header when permission is granted', () => {
    ...
    render(
      <Provider store={store}><PstnDetail /></Provider>
    );

    // Check DataPanel header
    expect(screen.getByTestId('PSTN Details')).toBeInTheDocument();
  });
});